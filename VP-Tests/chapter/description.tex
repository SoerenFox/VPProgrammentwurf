...
\textbf{Disclaimer} & This test used a different stack size configuration than the final requirements, but results remain valid for verifying the stack monitoring mechanism. See Figure~\ref{fig:StackMonitoring} and Figure~\ref{fig:StackMonitoringArray}. \\ \hline
\textbf{Status} & \textcolor{green!60!black}{Passed} \\ \hline
\end{tabular}
\end{table}
...
\textbf{Actual Result} & Corruption detected when array is filled, not when only reserved. See Figure~\ref{fig:stackoverflowEmpty} and Figure~\ref{fig:stackoverflowFilled}. \\ \hline
